---
timestamp: 'Sat Oct 18 2025 18:49:37 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251018_184937.08002b28.md]]'
content_id: 73505af21ed48d86f8005affce976f0392b0c1a316f3d7e15cece4e5ab922b7f
---
 ...
 * - 'ALL_MUSEUMS': Only compute links for museum items.
 * - 'ALL_EXHIBITS': Only compute links for exhibit items.
 * - 'ALL_ITEMS': Compute links for both museum and exhibit items (default).
 */
export type Scope = "ALL_MUSEUMS" | "ALL_EXHIBITS" | "ALL_ITEMS";
 ...
 *
 * @param {Scope} scope - The scope to filter which items are processed ('ALL_MUSEUMS', 'ALL_EXHIBITS', or 'ALL_ITEMS').
...
function processMuseumCatalog(scope: Scope): Map<ItemId, Set<string>> {
  ...
  for (const museum of museumsRaw as MuseumData[]) {
    ...
    if (scope === "ALL_MUSEUMS" || scope === "ALL_ITEMS") {
    ...
    if (scope === "ALL_EXHIBITS" || scope === "ALL_ITEMS") {
...
export default class SimilarityConcept {
  ...
  constructor(private readonly db: Db) {
    this.similarityLinks = this.db.collection(PREFIX + "similarityLinks");
    this.allItemsCatalog = processMuseumCatalog("ALL_ITEMS"); // Load all items for comprehensive validation
  }
   ...
   * @param {Scope} [params.scope='ALL_ITEMS'] - Optional. The scope to limit the rebuild.
   *                                            Valid values are 'ALL_MUSEUMS', 'ALL_EXHIBITS', or 'ALL_ITEMS'.
  ...
  async rebuildSimilarity(
    ...
    const currentScope: Scope = scope || "ALL_ITEMS";
