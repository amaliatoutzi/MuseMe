---
timestamp: 'Sun Oct 19 2025 12:10:00 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251019_121000.4472a020.md]]'
content_id: 4c7cea5969aa038f3b55532bdbdceabeab837fb4e7e2d58ad3569b4c9876c806
---
...
import { toID } from "@utils/types.ts";
import SavingConcept from "@concepts/Saving/SavingConcept.ts";

// --- Test Data ---
const USER_ALICE = toID("user:Alice");
const USER_BOB = toID("user:Bob");
const USER_CHARLIE = toID("user:Charlie");

// Valid item IDs from new-york-museums.json (example values)
const MUSEUM_STATEN_ISLAND = toID("staten-island-museum");
const MUSEUM_ALICE_AUSTEN = toID("alice-austen-house-museum");
const EXHIBIT_STORY_OF_HOUSE = toID("the-story-of-the-house"); // Exhibit of Alice Austen House
const MUSEUM_AMERICAN_FOLK = toID("american-folk-art-museum");

// Invalid item ID
const INVALID_ITEM = toID("invalid-museum-or-exhibit");
...
Deno.test("Saving Concept - Operational Principle", async (t) => {
  ...
  try {
    ...
    const saveResult1 = await saving.saveItem({ user: USER_ALICE, item: MUSEUM_STATEN_ISLAND });
    assert(!isError(saveResult1), `saveItem should succeed: ${JSON.stringify(saveResult1)}`);
    assertEquals(saveResult1, {}, "saveItem should return empty object on success");
    ...
    assert(!isError(aliceSaved), `_listSaved should succeed: ${JSON.stringify(aliceSaved)}`);
    assertEquals(aliceSaved.length, 1, "Alice should have 1 saved item");
    assertEquals(aliceSaved[0].item, MUSEUM_STATEN_ISLAND, "The saved item should be Staten Island Museum");
    ...
    const unsaveResult1 = await saving.unsaveItem({ user: USER_ALICE, item: MUSEUM_STATEN_ISLAND });
    assert(!isError(unsaveResult1), `unsaveItem should succeed: ${JSON.stringify(unsaveResult1)}`);
    assertEquals(unsaveResult1, {}, "unsaveItem should return empty object on success");
    ...
    assert(!isError(aliceSaved), `_listSaved should succeed: ${JSON.stringify(aliceSaved)}`);
    assertEquals(aliceSaved.length, 0, "Alice should have 0 saved items after unsaving");
    console.log(`Result: Alice's saved items: ${JSON.stringify(aliceSaved)}`);

    console.log("Operational Principle: Successfully demonstrated saving and unsaving an item.");
  } finally {
    await client.close();
  }
});
...
Deno.test("Saving Concept - Invalid Item ID Error", async (t) => {
  ...
  try {
    ...
    const saveResult = await saving.saveItem({ user: USER_BOB, item: INVALID_ITEM });

    assert(isError(saveResult), `saveItem should return an error for invalid item ID.`);
    assert(
    ...
    assert(!isError(bobSaved), `_listSaved should succeed: ${JSON.stringify(bobSaved)}`);
...
Deno.test("Saving Concept - Duplicate Save Error", async (t) => {
  ...
  try {
    ...
    const saveResult1 = await saving.saveItem({ user: USER_ALICE, item: MUSEUM_ALICE_AUSTEN });
    assert(!isError(saveResult1), `saveItem should succeed the first time: ${JSON.stringify(saveResult1)}`);

    // trace: Alice tries to save Alice Austen House Museum again
    console.log(`Action: ${USER_ALICE} attempts to save ${MUSEUM_ALICE_AUSTEN} again`);
    const saveResult2 = await saving.saveItem({ user: USER_ALICE, item: MUSEUM_ALICE_AUSTEN });

    assert(isError(saveResult2), `saveItem should return an error for duplicate save.`);
    assert(
    ...
    assert(!isError(aliceSaved), `_listSaved should succeed: ${JSON.stringify(aliceSaved)}`);
    assertEquals(aliceSaved.length, 1, "Only one entry for Alice Austen House Museum should exist.");
    assertEquals(aliceSaved[0].item, MUSEUM_ALICE_AUSTEN, "The saved item should be Alice Austen House Museum.");
...
Deno.test("Saving Concept - Unsave Non-Existent Item Error", async (t) => {
  ...
  try {
    ...
    console.log(`User ${USER_BOB} attempts to unsave an item they haven't saved.`);
    ...
    const unsaveResult = await saving.unsaveItem({ user: USER_BOB, item: MUSEUM_ALICE_AUSTEN });

    assert(isError(unsaveResult), `unsaveItem should return an error for non-existent save.`);
    assert(
    ...
    const bobSaved = await saving._listSaved({ user: USER_BOB });
    assert(!isError(bobSaved), `_listSaved should succeed: ${JSON_stringify(bobSaved)}`);
    assertEquals(bobSaved.length, 0, "No items should be saved for Bob.");
...
Deno.test("Saving Concept - Multiple Saves and Listing with Limit", async (t) => {
  ...
  try {
    ...
    console.log(`Action: ${USER_CHARLIE} saves ${MUSEUM_AMERICAN_FOLK}`);
    await saving.saveItem({ user: USER_CHARLIE, item: MUSEUM_AMERICAN_FOLK });
    // Add small delay to ensure distinct 'createdAt' for sorting
    await new Promise((resolve) => setTimeout(resolve, 10));
    console.log(`Action: ${USER_CHARLIE} saves ${EXHIBIT_STORY_OF_HOUSE}`);
    await saving.saveItem({ user: USER_CHARLIE, item: EXHIBIT_STORY_OF_HOUSE });
    await new Promise((resolve) => setTimeout(resolve, 10));
    console.log(`Action: ${USER_CHARLIE} saves ${MUSEUM_ALICE_AUSTEN}`);
    await saving.saveItem({ user: USER_CHARLIE, item: MUSEUM_ALICE_AUSTEN });
    ...
    assert(!isError(charlieSaved), `_listSaved should succeed: ${JSON.stringify(charlieSaved)}`);
    assertEquals(charlieSaved.length, 3, "Charlie should have 3 saved items.");
    // Verify order (newest first)
    assertEquals(charlieSaved[0].item, MUSEUM_ALICE_AUSTEN, "Newest item should be Alice Austen House Museum.");
    assertEquals(charlieSaved[1].item, EXHIBIT_STORY_OF_HOUSE, "Second newest should be The Story of the House.");
    assertEquals(charlieSaved[2].item, MUSEUM_AMERICAN_FOLK, "Oldest item should be American Folk Art Museum.");
    console.log(`Result: Charlie's all saved items: ${JSON.stringify(charlieSaved.map(s => s.item))}`);
    ...
    assert(!isError(charlieSavedLimited), `_listSaved with limit should succeed: ${JSON.stringify(charlieSavedLimited)}`);
    assertEquals(charlieSavedLimited.length, 2, "Charlie should have 2 saved items with limit.");
    assertEquals(charlieSavedLimited[0].item, MUSEUM_ALICE_AUSTEN, "First item should be Alice Austen House Museum.");
    assertEquals(charlieSavedLimited[1].item, EXHIBIT_STORY_OF_HOUSE, "Second item should be The Story of the House.");
...
Deno.test("Saving Concept - Mixed User Saves and Independent Lists", async (t) => {
  ...
  try {
    ...
    assertEquals(aliceSaved.length, 1, "Alice should have 1 saved item.");
    assertEquals(aliceSaved[0].item, MUSEUM_STATEN_ISLAND, "Alice's item should be Staten Island Museum.");
    console.log(`Result: Alice's saved items: ${JSON.stringify(aliceSaved.map(s => s.item))}`);
    ...
    assertEquals(bobSaved[0].item, MUSEUM_AMERICAN_FOLK, "Bob's newest item should be American Folk Art Museum.");
    assertEquals(bobSaved[1].item, MUSEUM_STATEN_ISLAND, "Bob's older item should be Staten Island Museum.");
    console.log(`Result: Bob's saved items: ${JSON_stringify(bobSaved.map(s => s.item))}`);

    // trace: Bob unsaves Staten Island Museum
    console.log(`Action: ${USER_BOB} unsaves ${MUSEUM_STATEN_ISLAND}`);
    const bobUnsaveResult = await saving.unsaveItem({ user: USER_BOB, item: MUSEUM_STATEN_ISLAND });
    ...
    assertEquals(bobSaved[0].item, MUSEUM_AMERICAN_FOLK, "Bob's remaining item should be American Folk Art Museum.");
    console.log(`Result: Bob's remaining saved items: ${JSON_stringify(bobSaved.map(s => s.item))}`);

    // trace: Verify Alice's list is unaffected
    console.log(`Query: List saved items for ${USER_ALICE} (should be unaffected)`);
    aliceSaved = await saving._listSaved({ user: USER_ALICE });
    ...
    assertEquals(aliceSaved[0].item, MUSEUM_STATEN_ISLAND, "Alice's item should still be Staten Island Museum.");
    console.log(`Result: Alice's saved items: ${JSON_stringify(aliceSaved.map(s => s.item))}`);

    console.log("Mixed User Saves: Successfully demonstrated independent saving and lists.");
  } finally {
    await client.close();
  }
});
