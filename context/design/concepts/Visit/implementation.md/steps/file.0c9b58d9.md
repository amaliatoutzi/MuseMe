...
content_id: 0c9b58d9a34fb2f00132f06a87e13b04731f18b62fd4e5a6e60ae3a42f7755ed
---
...
export default class VisitConcept {
  ...
  async createVisit(
    ...
    if (!this.museumCatalog.has(museum)) {
      return { error: "Museum not found in catalog." };
    }
  ...
  async addEntry(
    ...
    if (!museumExhibits || !museumExhibits.has(exhibit)) {
      return { error: "Exhibit not found or does not belong to the visit's museum." };
    }
  ...
  /**
   * editEntry(visit: VisitId, visitEntryId: VisitEntryId, note?: String, photoUrl?: String, user: User) : Empty | { error: String }
   ...
   */
  async editEntry(
    { visit: visitId, visitEntryId, note, photoUrl, user }: {
      visit: VisitId;
    ...
    const entry = await this.visitEntries.findOne({ _id: visitEntryId, visit: visitId });
    if (!entry) {
      return { error: "Visit entry not found for the specified visit." };
    }
    ...
    const now = new Date();
    const updateFields: { note?: string; photoUrl?: string; updatedAt: Date } = {
      updatedAt: now,
    };
  ...
  /**
   * removeEntry(visit: VisitId, visitEntryId: VisitEntryId, user: User) : Empty | { error: String }
   * requires: entry exists and user = entry.visit.owner
   * effects: delete the entry; set entry.visit.updatedAt := now
   */
  async removeEntry(
    { visit: visitId, visitEntryId, user }: {
      visit: VisitId;
      visitEntryId: VisitEntryId;
      user: User;
    },
